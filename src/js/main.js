document.addEventListener('DOMContentLoaded', () => {
    ...
    const togglePasswordButtons = document.querySelectorAll('button i.ph-eye-slash, button i.ph-eye');
    ...
    document.addEventListener('click', (e) => {
        if (!e.target.closest('[data-dropdown-content]') && !e.target.closest('[data-dropdown-trigger]')) {
            document.querySelectorAll('[data-dropdown-content]').forEach(d => {
    ...
    const chatInput = document.querySelector('input[placeholder="Type your response..."]');
    const sendButton = chatInput ? chatInput.nextElementSibling : null;
    const chatContainer = document.querySelector('#chat-messages-container') || document.querySelector('.overflow-y-auto.space-y-6');
    ...
    const chatData = {
        'chat_1': {
            ...
            role: 'Brand',
            messages: [
                { type: 'received', text: "Hi! I'm here to help you create a detailed product brief. Let's start with the basics. What type of product are you looking to manufacture?", time: '20:32' }
            ]
        },
        'chat_2': {
            id: 'chat_2',
            ...
            role: 'Brand',
            messages: [
                { type: 'received', text: "Yes, customization is available with a 20-day lead time for this specific fabric blend. Does that work for your timeline?", time: '09:15' },
                { type: 'sent', text: "Can we expedite that to 15 days if we increase the order volume?", time: '09:20' }
            ]
        },
        'chat_3': {
            id: 'chat_3',
            ...
            role: 'Brand',
            messages: [
                { type: 'received', text: "We've reviewed your brief and sent a quote matching your requirements. Please check it and let us know if you have questions.", time: 'Yesterday' }
            ]
        }
    };
    ...
    function switchChat(chatId) {
        ...
        document.querySelectorAll('[data-chat-id]').forEach(el => {
            if (el.dataset.chatId === chatId) {
                el.className = 'p-4 bg-blue-50/50 border-l-4 border-blue-600 cursor-pointer';
                el.querySelector('.font-bold').classList.remove('text-slate-700'); // Ensure specific styling if needed
            } else {
                el.className = 'p-4 hover:bg-gray-50 cursor-pointer border-l-4 border-transparent';
            }
        });
    ...
    if (chatInput && sendButton && chatContainer) {
        ...
        const handleSendMessage = () => {
            ...
            if (messageText) {
                ...
                setTimeout(() => {
                    const aiResponseText = "Thanks for your message. We'll get back to you shortly.";
                    // Add to data model
                     if (chatData[activeChatId]) {
                        chatData[activeChatId].messages.push({
                            type: 'received',
    ...
    const currentPath = window.location.pathname.split('/').pop() || 'dashboard-filled.html';
    const navLinks = document.querySelectorAll('nav a');
    
    navLinks.forEach(link => {
        const href = link.getAttribute('href');
        if (href === currentPath) {
             // Ensure active styling is present
             link.classList.remove('text-slate-500', 'hover:text-slate-900');
