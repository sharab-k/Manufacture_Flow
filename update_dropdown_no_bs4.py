...
dropdown_content = """<!-- Notification Dropdown -->
...
for filename in html_files:
    ...
    start_str = '<!-- Notification Dropdown -->'
    start_idx = content.find(start_str)
    
    if start_idx != -1:
        # Find the end of the original dropdown. The original dropdown structure is:
        ...
        # Or if it was already updated previously because we rolled back... wait, we rolled back so it's the original.
        end_marker = '<p class="text-xs text-slate-500 md:text-sm mt-4">No new notifications</p>'
        end_idx = content.find(end_marker, start_idx)
        
        if end_idx != -1:
            div_close_idx = content.find('</div>', end_idx) + 6
            new_content = content[:start_idx] + dropdown_content + content[div_close_idx:]
